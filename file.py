...
num1 = 42 # variable declaration, primitive data type, number, integer
num2 = 2.3 # variable declaration, primitive data type, number, float
boolean = True # variable declaration, primitive data type, boolean
string = 'Hello World' # variable declaration, prmitive data type, string
pizza_toppings = ['Pepperoni', 'Sausage', 'Jalepenos', 'Cheese', 'Olives'] # list initialization, composite data type, list, list of primitive data type, string
person = {'name': 'John', 'location': 'Salt Lake', 'age': 37, 'is_balding': False} # dictionary initialization, composite data type, dictionary, dictionary of primitive data types, string, number, integer, boolean
fruit = ('blueberry', 'strawberry', 'banana') # tuple initialization, composite data type, tuple, tuple of primitive data type, string
...
print(person['name']) # built in function, outputs value within parenthesis to console, access's value asscoiated with key named 'name' within dictionary named person
person['name'] = 'George' # Changes value associated with key named 'name', within dictionary named person, from current value to 'George'
person['eye_color'] = 'blue' # Adds value 'blue' and asscoaited key named 'eye_color' to dictionary named person, as a key value pair
print(fruit[2]) # built in function, outputs value within parenthesis to console, access value of tuple named fruit at index 2 which is 3rd position of ttuple

if num1 > 45: # Condition, If statement, compares value of variable num1 to the primitive data type, number, integer 45
    print("It's greater") # If Condition, is true, built in function will output value within parenthesis
else: # Condition, else statement
    print("It's lower") # If Condition, is false, when all conditions are false built in function will output value within parenthesis
...
print(person) # built in function, outputs value within parenthesis to the console, composite data type, dictionary named person
person.pop('eye_color') # built in method, chooses value associated with key named 'eye color', removes value and key from dictionary, and outputs the same value that was removed out to the console
print(person) # built in function, outputs value within parenthesis to the console, composite data type, dictionary named person

for topping in pizza_toppings: # for loop, variable declaration, intialization
    if topping == 'Pepperoni': # Condition, if statement, primitive data type, string
    ...
    print('After 1st if statement') # built in function, out puts value in parenthesis to console
    if topping == 'Olives': # Condition, If Statement, primitive data type, string
...
def print_hello_ten_times(): # function, creates function with no parameters named print_hello_ten_times
    for num in range(10): # for loop, will run loop for range start at 0, end at 9, variable declaration, of variable named num, for every iteration of loop
        print('Hello') # built in function, out puts the value in parenthesis to the console for every iteration of the loop
...
def print_hello_x_times(x): # function, creates function with one parameter, x, named print_hello_x_times
    for num in range(x): # for loop, will run loop for range, start at 0 and end at x minus 1, variable declaration, of variable named num, for every iteration of loop
        print('Hello') # built in function, out puts value within parenthesis to the console for every iteration of the loop

print_hello_x_times(4) # function call with one argument, 4, executes the function named print_hello_x_times

def print_hello_x_or_ten_times(x = 10): # function, creates function with one parameter, x = 10, where variable x is already declared and intialized to the value 10, but can also receive a new argument, for function  named print_hello_x_or_ten_times
    for num in range(x): # for loop, will run loop for range, start at 0 and end at 9, when no argument is entered and start at 0 and end at x minus 1 when an argument is entered, variable declaration, of variable named num, for every iteration of loop
        print('Hello') # built in function, out puts value within parenthesis to the console for every iteration of the loop
...
print_hello_x_or_ten_times(4) # function call with one argument, 4, executes the function named print_hello_x_or_ten_times will print Hellofour times
